...
@misc{teststandard,
	...
	title			={{IEEE 829-2008 - IEEE Standard for Software and System Test Documentation}},
	note			={\url{https://standards.ieee.org/standard/829-2008.html}
	}	
	...
	\begin{tabular}{|l|l|l|l|}
	 ...
	 4 & 4/12/19  	 & Heather Libecki 		   & Changed the Arduino Uno 3 microcontrollers and  \\ 
 	 ...
 	 5 & 4/21/19  	 & Heather Libecki		   & Updated Milestone Schedule, GANTT Charts, and \\ 
 	    ...
 	    \hline
 	 6 & 4/21/19  	 & Heather Libecki		   & Put finishing touches on each section in Section~\ref{AnD}\\ 
 	    &         		 & Chris Quesada  		   & of the report.\\
 	    &				 & Juan Colin			& \\
...
Figure~\ref{fig:rough} shows an overview of the final deliverable's mechanical and electrical components assembled with names of each item. These components and a small description of their role are listed next. For a more detailed description of each component and its role, see Section~\ref{PDB} of this report.
...
This section describes the objectives of the Smart Eraser. A more detailed description of the project can be found in Section~\ref{PDB} of this report.
...
Based on the courses provided in the previous list, the following information was extracted from the knowledge gained while taking these classes over the last 3-4 years. Because the following information is from notes taken during these courses, there are no sources provided for where it was learned. More in depth research obtained from external sources and their references are located in Section~\ref{PDB} of this report.\\
...
Additional research pertaining to these topics and how they were implemented in the project are in Section~\ref{PDB} of this report.   
...
\begin{itemize}
	...
	\end{itemize}
	\item SPI Standard of Communication
	\item I\textsuperscript{2}C Standard of Communication
	...
	\end{itemize}
	\item IEEE 829-2008 - IEEE Standard for Software and System Test Documentation \cite{teststandard}
	\begin{itemize}
		\item Applies to organization and carry-out of test plan and other testing documentation.
	\end{itemize}
\end{itemize}
 
\section{Project Description and Boundaries} \label{PDB}
This section contains a list of the major and minor components that are used in this project, as well as a quick overview of each of these components and how they play a role in the Smart Eraser project, and some boundaries that were overcome in order to ensure the success of this project. For a more detailed description of the procedure followed and the actual implementation of each part, see Section~\ref{AnD} of this report. \par
\setlength{\parindent}{2.5ex}The following lists contain the major and minor components used in this project. More details about each major component's specifications and model numbers can be found in Section~\ref{EB} of this report.
...
The communication of choice between two micro controllers is often UART as it allows for the simplest method to communicate. Since there is no master and no slave, packet transmission is done through 1 receiver and 1 transmitter. Hardware was configured according to the RS-232 standard of serial communication \cite{UARTstandard} which means there is only 1 transmitter and one receiver on both ends. The communication mode is full duplex which allows for communication in both directions as well as at the same time. The interfaces of both will expect a baud rate of 9600, parity of even, 1 stop bit and a data frame that is 8 bits long.
\begin{figure}[H]
	\centering
...
The motor moves when these steps are sent to their corresponding lead connections to the stepper motor drivers in a sequential order. The factor that determines how quickly the motors turn in the code that was written is how quickly the steps are traversed through when they are sent to the drivers. I.E., the faster they are traversed, the faster the motor will spin, and visa versa.\\
 ...
 \setlength{\parindent}{2.5ex}
 In order to create the correct rotation sequence for each motor, the serpentine code had to be split into two separate files for each motor and modified accordingly. For more information on this implementation, see Section~\ref{wireless} of this report.
...
\setlength{\parindent}{2.5ex}
The dimensions of the whiteboard prototype are shown in the next two figures. More information and details about how the system was created, built, and complete can be found in Section~\ref{mount} of this report.
...
\begin{table} [H]	
	...
	\begin{tabular}{|l|l|l|}
		...
		\hline
		Juan C. & 1/6/19-1/16/19	 	& Assist in research of how to get Linux onto DE1\_SoC Board; \\
				&						& research the micro SD card needed and the how to implement the \\
				&						& QSYS system to run with Linux. \\
		\hline
...
\section{Equipment and Budget} \label{EB}
...
\section{Analysis and Design} \label{AnD}
...
In the file Arducam.C, found in ~\ref{A8}, MyCam\_Init() uses the Write\_I2C() function to send configuration signals to the image sensor of the Arducam module. These configurations are stored in structures in the beginning of the code file. As can be seen, just in the first configuration structure alone, there are over 250 configurations that need to be sent to the image sensor to get a RAW 1280x960 image stored to the FIFO buffer of the Arducam module. This is followed by another, much smaller set of configurations that is meant to resize the image. Due to the inability to set the output to RGB565, an attempt to receive RAW data was attempted and is the reasoning for sending theses configurations. Receiving RAW data requires another component to be added to the image processing, demosaicing, and is currently still in development. The Write\_I2C() function uses a combination of APIs provided by Cypress in order to interact with Serial Control Block 3 (SCB3) configured to be a I\textsuperscript{2}C master. It can be found in the section ~\ref{A6}.
 ...
 \begin{figure}[H]
	...
	\caption{Hardware Configuration of I2C using Modus Toolbox}
...
The Write\_SPI() function uses a combination of the APIs provided by Cypress in order to interact with the Serial Control Block 1(SCB1) that has been configured as aa SPI master. It is located in the I2Cmaster.C file found in ~\ref{A4}.
 ...
 \begin{figure}[H]
	...
	\caption{Hardware configuration of SPI in Modus Toolbox (1)}
...
\begin{figure}[H]
	...
	\caption{Hardware configuration of SPI in Modus Toolbox (2)}
...
The actual code for this algorithm has been completed using a ``dummy'' matrix with random values of 0s and 1s representing blank space and markings. The code for this is in Appendix~\ref{A3} of this report. The result with this dummy matrix is shown in Figure~\ref{fig:square_result}. This program is then combined with the \textit{serpentine.c} algorithm explained in Section~\ref{serp} of this report.
...
\begin{figure}[H]
	\centering
	{\includegraphics[max size={\textwidth}{\textheight}, scale=.95]{Images/UARThardware.png}}
	\caption{Hardware configuration of UART SCB using Modus Toolbox}
	\label{fig:UARThardware}
\end{figure}
...
\subsection{The Current Wireless Communication System} \label{wireless}
...
\subsubsection{\textit{Serpentine.c} Algorithm} \label{serp}
...
\subsubsection{Stepper Motor Movement} \label{SMM}
...
\begin{figure}[H]
	...
	\caption{Rotor and coil layout in the bipolar 2-phase NEMA 23 stepper motor \cite{smdataR}}
	\label{fig:rotor}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Labeling of lead colors corresponding to winding inputs \cite{smdataR}}
	\label{fig:bipolar}
\end{figure}
...
\begin{figure}[H]
	\centering
	{\includegraphics[max size={\textwidth}{\textheight}, scale=0.5]{Images/front_view.png}}
	\caption{The front view of the prototype stand schematic with dimensions}
	\label{fig:sideview}
\end{figure}
...
\subsubsection{Component Mounting Steps} \label{mount}
...
\subsection*{Appendix 21 - \textit{UART.py}}\label{A21}
\addcontentsline{toc}{section}{Appendix 21 - \textit{UART.py}}

\lstinputlisting{Appendices/UART.py}
