...
\noindent Gx =(sobel\_kernel[0])*(*(ptr\_GD - Length + 1)) + (sobel\_kernel[1])*(*(ptr\_GD + 1)) 	            +\\
(sobel\_kernel[2])*(*(ptr\_GD + Length + 1)) - (sobel\_kernel[0])*(*(ptr\_GD - Length - 1))    	-\\
(sobel\_kernel[1])*(*(ptr\_GD - 1))			 - 	(sobel\_kernel[2])*(*(ptr\_GD + Length - 1));\\

\noindent Gy =(sobel\_kernel[0])*(*(ptr\_GD - Length + 1)) + (sobel\_kernel[1])*(*(ptr\_GD - Length)) 		+\\
(sobel\_kernel[2])*(*(ptr\_GD - Length - 1)) - (sobel\_kernel[0])*(*(ptr\_GD + Length + 1))    	-\\
(sobel\_kernel[1])*(*(ptr\_GD + Length))	 - 	(sobel\_kernel[2])*(*(ptr\_GD + Length - 1)); \\

\noindent G = (sqrt(pow(Gx,2) + pow(Gy,2)));\\\\
\textit{Code is from sobel() in ImProc.C located in section ~\ref{A10}}\\
...
\begin{figure}[H]
	\centering
	{\includegraphics[max size={\textwidth}{\textheight}, scale=.65]{Images/sobelResult.png}}
	\caption{Results of Sobel edge detection}
	\label{fig:sobelResults}
\end{figure}

\subsubsection{UART Communication - PSoC 6 to HUZZAH32 (server)}
...
Cy\_SCB\_UART\_PutArray(KIT\_UART\_HW, ptr\_PD, 25);\\
Cy\_SysLib\_Delay(1);\\
Cy\_SCB\_UART\_ClearTxFifoStatus(KIT\_UART\_HW, CY\_SCB\_UART\_TX\_DONE);\\\\
...
For the code of the transmitter, the template code was taken from the example shown by Dejan Nedelkovski, and is shown in Appendix 1 of this report. In order to use the wireless transceiver, the specific library made for it needed to be included. The RF24 radio was then configured to use pins 6 and 7 on the Arduino as the CE and CSN pins shown in ~\ref{fig:trans1}. In the setup of the program, the radio connection is initialized, the PA level is set to the minimum it can be, and the radio is set to be a transmitter with the stopListening() function. The addresses the transmitter will be sending data to are also outlined. The PA is the Power Amplifier factor, and the higher it is, the more unstable it can be, which would then require a bypass capacitor to be connected between the voltage power source and ground connected to the chip. Therefore, it was kept to a minimum, especially because the modules were so close together, so it was unnecessary to amplify it more.\par
\setlength{\parindent}{2.5ex}
...
\subsubsection{Square\_Detection.C Algorithm}
...
The actual code for this algorithm has been completed using a ``dummy'' matrix with random values of 0s and 1s representing blank space and markings. The code for this is in \textit{Appendix 3} of this report. The result with this dummy matrix is shown in the following figure. This program is then combined with the \textit{serpentine.c} algorithm explained in the next section.

\begin{figure}[H]
	\centering
	{\includegraphics[max size={\textwidth}{\textheight}, scale=0.6]{Images/square_dummy_result.png}}
...
\begin{figure}[H]
	\centering
	{\includegraphics[max size={\textwidth}{\textheight}, scale=0.6]{Images/square_dummy_result.png}}
	\caption{Result of \textit{serpentine.c} program with the dummy matrix introduced within it for testing}
	\label{fig:serp_result}
\end{figure}
...
\begin{figure}[H]
	\centering
	{\includegraphics[max size={\textwidth}{\textheight}, scale=0.6]{Images/smD.png}}
...
The following images show the initial conception of the prototype stand, as well as some rough dimensions.
...
After the initial conception of the prototype stand, more definitive dimensions were found for the different components that would be connected to the board. The following figures show the more detailed dimensions, as well as a more detailed figure that shows where everything would be placed relative to each other.
...
The final product in progress showing how the components were mounted to the front of the board, as well as the finished final product, are shown in the following figures.
...
\subsection*{Appendix 17 - \textit{SMclient.py}}\label{A17}
\addcontentsline{toc}{section}{Appendix 17 - \textit{SMclient.py}}

\lstinputlisting{Appendices/SMclient.py}

\newpage

\subsection*{Appendix 18 - \textit{ultrasonic.py}}\label{A18}
\addcontentsline{toc}{section}{Appendix 18 - \textit{ultrasonic.py}}

\lstinputlisting{Appendices/ultrasonic.py}

\newpage

\subsection*{Appendix 19 - \textit{np\_lights.py}}\label{A19}
\addcontentsline{toc}{section}{Appendix 19 - \textit{np\_lights.py}}
